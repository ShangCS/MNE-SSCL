...
def evaluate(embeds, idx_train, idx_val, idx_test, labels, dataset, isTest=True):
    ...
    nmi = run_kmeans(test_embs, test_lbls, nb_classes, dataset)
    sim = run_similarity_search(test_embs, test_lbls, dataset)
    return float(format(float(np.mean(macro_f1s)), "0.4f")), float(format(float(np.mean(micro_f1s)), "0.4f")), float(format(float(nmi[0]), "0.4f")), sim
